    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
      <div v-for="h in holdings" :key="h.name" data-holding-card class="rounded-2xl border border-[var(--vp-c-divider)] p-5 bg-[var(--vp-c-bg)]">
        <div class="flex items-baseline justify-between">
          ...
          <span class="text-sm text-[var(--vp-c-text-2)]">{{ h.kind }}</span>
        </div>
        <p class="text-sm text-[var(--vp-c-text-2)] mt-2">{{ h.note[lang] }}</p>
        <a v-if="h.disclosure_url" :href="h.disclosure_url" class="text-sm text-[var(--vp-c-brand-1,#00b8b8)] mt-3 inline-block" target="_blank" rel="noopener">
          Source →
        </a>
      </div>
    </div>
